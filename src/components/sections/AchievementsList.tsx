...
function AchievementNode({
  ...
  return (
    <div ref={ref} className="relative border-t border-[var(--color-line)] pt-5">
      <motion.span
        aria-hidden
        className="absolute -top-[3px] left-0 h-1.5 w-1.5 rounded-full bg-[var(--color-accent)]"
      ...
      <div className="flex items-baseline justify-between gap-3">
        ...
        {year && <span className="shrink-0 font-mono text-xs text-[var(--color-mist)]">{year}</span>}
      </div>
      {org && <p className="mt-1 text-sm text-[var(--color-ink-soft)]">{org}</p>}
      {detail && <p className="mt-2 text-sm leading-relaxed text-[var(--color-mist)]">{detail}</p>}
    </div>
  );
}
